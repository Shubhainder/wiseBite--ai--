function scrapeData() {
  ...
  // swiggy
    ...
    if (
      ingredientsElement &&
      ingredientsElement.innerText.toLowerCase().includes("ingredients")
    ) {
  ...
  // zepto - now  (zeptonow.com)
    ...
    const title = document
      .querySelector('[data-testid="pdp-product-name"]')
      ?.innerText.trim();
    if (title) {
      scrapedText += `Product Title: ${title}\n\n`;
      finalTitle=title;
  ...
  // blinkit
    ...
    const title = document.querySelector(".dsuWXl")?.innerText.trim();
    if (title) {
      scrapedText += `Product Title: ${title}\n\n`;
      finalTitle=title;
  ...
  // bigbasket
    ...
    const brand = document
      .querySelector(".Brand___StyledH-sc-zi64kd-1")
      ?.innerText.trim();
    if (brand) {
    ...
    const moreDetails = document.querySelector(
      ".MoreDetails___StyledSection-sc-1h9rbjh-4"
    );
    if (moreDetails) {
...
sendFinalTitle(finalTitle)
  console.log(scrapedText);
  return {
    text: scrapedText || document.body.innerText.slice(0, 1000),
    images,
  };
}
...
function sendFinalTitle(finalTitle){

  
  chrome.storage.local.set({ productTitle: finalTitle }, () => {
    console.log('Product title saved to storage');
  });
}
